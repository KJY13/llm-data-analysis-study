...
- 기준: 월별
- 비교 방법: 최근 6개월 월별 변화
- 분석 목적: 각 제품 카테고리별 환불률 탐색 및 top5 선정

### 분석 가능한 질문으로 다시 작성

```text
각 제품 카테고리별 환불률은 최근 6개월 간 월별로 어떻게 변했으며,
월별로 가장 높은 환불률을 보이는 5개 카테고리은 무엇인가?
```

### 결과 관찰

모호하던 질문이 이해에 있어 오차가 없는 질문으로 변하였다.

### 나의 해석과 판단

사람마다 질문을 보고 이해하는 것이 달라지지 않았으며,
이로 인해 의사소통이 더 효율적으로 될 수 있을 것이다.

### 업무·분석적 의미

이해하는 내용이 달라진다면 해당 갭을 채우기 위해 비용이 발생한다.
따라서 최선의 방법은 처음부터 모두가 정확히 이해할 수 있는 질문을 제시하는 것이다.

### 한계와 추가 확인 사항

각 제품별로 월별 환불률을 따져봤을 때, 해석에 있어 유의미한 결과값이 나올지 알 수 없다.
...
| 파일 | 필요한 컬럼 | 필요한 이유 |
| --- | --- | --- |
| products.csv | product_id | order_items.csv와 연결 |
| products.csv | category | 최종 도출값에 포함 |
| orders.csv | order_id | order_items.csv와 연결 |
| orders.csv | order_date | 월별 분류에 필요 |
| orders.csv | order_status | 환불률 계산 |
| order_items.csv | order_id | orders.csv와 연결 |
| order_items.csv | product_id | products.csv와 연결 |

### 데이터 연결 관계

```text
products.csv와 orders.csv가 order_items.csv를 통해 연결된다.
이 때 고유값인 order_id와 product_id가 사용된다.
```

### 결과 관찰

월별 분류를 위한 구매 날짜와 최종 도출값이 될 예정인 카테고리, 각 카테고리 별 환불률이 필요하다.

### 나의 해석과 판단

환불률을 위해 따로 계산 작업이 필요하다.

### 업무·분석적 의미

전체적인 구조를 머릿속에 그린 후 시행해야 오류 없이 분석을 진행할 수 있기 때문이다.

### 한계와 추가 확인 사항

환불률 계산을 위한 추가 데이터 분석 작업을 고려하지 않았다.
